...
int main()
{
    ...
    cout << "Enter the roads (u, v, type):" << endl;
    for (int i = 0; i < M; i++)
    {
        int u, v, type;
        cin >> u >> v >> type;
    ...
    // Process type 3 edges first (both men and women)
    ...
    // Process type 1 (men) and type 2 (women) edges
    for (auto [u, v, type] : edges)
    {
        if (type == 1)
        {
    ...
    // Check if all cities are connected for men and women
    bool fullyConnectedMen = true, fullyConnectedWomen = true;
    for (int i = 1; i <= N; i++)
    {
        if (men.find(i) != men.find(1))
            fullyConnectedMen = false;
        if (women.find(i) != women.find(1))
            fullyConnectedWomen = false;
    }
